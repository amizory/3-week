...
->  Jenkins-агент - это машина или контейнер, который выполняет задания 
    Jenkins. Агент может быть физическим или виртуальным, и он может быть
    расположен где угодно - в локальной сети или в облаке.

    1-Перейти в раздел "Управление Jenkins" -> "Узлы" (Nodes) в веб-интерфейсе Jenkins.
    2-Нажать кнопку "Новый узел" (New Node).
    3-Ввести имя и описание узла (агента).
    4-Выбрать тип узла:
        - "Permanent Agent" - постоянный агент, который всегда доступен.
        - "Temporary Agent" - временный агент, который создается только для
        выполнения конкретного задания.
        - "Docker Agent" - агент, который запускается в контейнере Docker.
    5-Настроить параметры подключения:
        - Количество процессов-исполнителей, использование (на сколько загружать)
        - Адрес узла (агента)
        - Порт подключения
        - Протокол подключения (например, SSH или JNLP)
        - Свойства узла 
            - Disk Space Monitoring Thresholds
            - Переменные среды
            - Расположения инструментов
    6-Настроить автономный режим:
        - Время ожидания до отключения агента
        - Время ожидания до повторного подключения агента
    7-Сохранить изменения.
    После добавления нового агента, Jenkins будет использовать его для
    выполнения заданий. Агент может быть использован для выполнения различных
    задач, таких как сборка и тестирование кода, деплоймент приложений и т. д 
```

```txt
2 - Почему не рекомендуется запускать пайплайны на мастере?

->  - Нагрузка на мастер: Мастер отвечает за управление всеми узлами и
    пайплайнами в Jenkins. Если пайплайны будут запускаться на мастере,
    это может привести к перегрузке и снижению производительности всего системы.
    - Безопасность: Мастер имеет доступ к конфиденциальной информации,
    такой как пароли и ключи. Если пайплайны будут запускаться на мастере,
    это может увеличить риск утечки конфиденциальной информации.
    - Скалабельность: Если пайплайны будут запускаться на мастере, это может
    ограничить возможности масштабирования системы, поскольку мастер может
    стать узким местом системы, если количество пайплайнов увеличится.
    - Восстановление после сбоя: Если пайплайны будут запускаться на мастере
    и мастер упадет, все пайплайны будут остановлены. Если пайплайны будут
    запускаться на отдельных узлах, они могут продолжать работать даже если
    мастер упадет.
    - Изоляция: Запуск пайплайнов на отдельных узлах позволяет изолировать их
    друг от друга, что снижает риск влияния одного пайплайна на другой.
```

```txt
3 - Что такое Job, Build и pipeline в Jenkins? Какая между ними взаимосвязь?

->  В Jenkins, Job, Build и Pipeline являются основными понятиями,
    которые используются для управления процессом сборки и тестирования
    программного обеспечения.

    - Job: Job (задание) - это основная единица работы в Jenkins. Это может
    быть сборка программного обеспечения, тестирование, деплоймент или любая
    другая задача, которую необходимо выполнить. Job может быть создан для
    выполнения конкретной задачи, такой как сборка программного
    обеспечения, тестирование или деплоймент.
    - Build: Build (сборка) - это конкретный запуск Job. Каждый раз,
    когда Job запускается, создается новая сборка. Сборка может быть успешной
    или неудачной, в зависимости от результатов выполнения Job.
    - Pipeline: Pipeline (конвейер) - это серия Job, которые выполняются
    последовательно. Pipeline позволяет автоматизировать процесс сборки,
    тестирования и деплоймента программного обеспечения. Pipeline может
    состоять из нескольких Job, которые выполняются в определенной последовательности.

    Взаимосвязь между Job, Build и Pipeline следующая:
    - Job - это основная единица работы в Jenkins.
    - Build - это конкретный запуск Job.
    - Pipeline - это серия Job, которые выполняются последовательно.

    Например, можно создать Job для сборки ПО, а затем создать Pipeline, который
    включает в себя этот Job, а также Job для тестирования и деплоймента. Каждый раз,
    когда Pipeline запускается, создается новая сборка для каждого Job в Pipeline.

    Pipeline: "Сборка и деплоймент программного обеспечения"
        Job 1: "Сборка программного обеспечения"
            Build 1: "Сборка программного обеспечения" (успешная)
        Job 2: "Тестирование программного обеспечения"
            Build 2: "Тестирование программного обеспечения" (успешная)
        Job 3: "Деплоймент программного обеспечения"
            Build 3: "Деплоймент программного обеспечения" (успешная)
```

```txt
4 - Какие виды синтаксиса пайплайнов поддерживает Jenkins? В чем отличия между ними?

->  - Declarative Pipeline: Этот синтаксис был введен в Jenkins 2.0 и является
    рекомендуемым способом описания пайплайнов. Declarative Pipeline использует
    синтаксис Groovy, но имеет более простой и читаемый формат, чем Scripted Pipeline.
    ...
    - Использует ключевые слова для описания пайплайна (например, pipeline,
    agent, stages)
    - Поддерживает автоматическое создание пайплайна на основе файла Jenkinsfile
    - Более безопасный, поскольку использует ограниченный набор функций Groovy

    - Scripted Pipeline: Этот синтаксис был доступен в более ранних
    версиях Jenkins и позволяет описывать пайплайны с помощью Groovy-скриптов.
    Scripted Pipeline более гибкий, чем Declarative Pipeline, но требует более
    глубоких знаний Groovy
    ...
    Scripted Pipeline
    - Более гибкий и мощный синтаксис
    - Использует Groovy-скрипты для описания пайплайна
    - Требует более глубоких знаний Groovy
    - Поддерживает более сложные пайплайны, но требует более тщательного
    тестирования

    Когда использовать Declarative Pipeline:
        Для простых пайплайнов, которые не требуют сложной логики
        Для пайплайнов, которые должны быть легко читаемы и понятны
    Когда использовать Scripted Pipeline:
        Для сложных пайплайнов, которые требуют гибкой логики
        Для пайплайнов, которые должны быть интегрированы с другими
        системами или инструментами
```

```txt
5 - Какие есть тригеры запуска пайплайнов в Jenkins?

->  В Jenkins есть три основные триггера запуска пайплайнов:
    - Timer Trigger: Этот триггер позволяет запускать пайплайны по
    расписанию, например, каждый день в определенное время.
    - Webhook Trigger: Этот триггер позволяет запускать пайплайны при получении
    HTTP-запроса от внешнего источника, например, от GitHub или GitLab.
    - SCM Trigger: Этот триггер позволяет запускать пайплайны при изменении кода
    в репозитории, например, при коммите или пуше в Git.
    
    Также есть другие триггеры, такие как:
    - Manual Trigger: Этот триггер позволяет запускать пайплайны
    вручную, например, при нажатии кнопки в интерфейсе Jenkins.
    - API Trigger: Этот триггер позволяет запускать пайплайны
    через API Jenkins.
    - Script Trigger: Этот триггер позволяет запускать пайплайны через
    скрипт, написанный на языке Groovy
```
...
->  В Jenkins, секреты (secrets) - это конфиденциальная информация,
    такая как пароли, ключи и токены, которые используются для аутентификации и
    авторизации. Чтобы работать с секретами в Jenkins, можно использовать
    следующие методы:
    Credential Providers
    - Credentials: Jenkins предоставляет возможность хранить секреты в виде
    учетных данных (credentials). Это можно сделать в - разделе "Управление
    Jenkins" -> "Учетные данные" (Credentials).
    Environment Variables: Секреты можно хранить в виде переменных окружения
    (environment variables). Это можно сделать в разделе "Управление Jenkins"
    -> "Конфигурация" (Configure) -> "Переменные окружения" (Environment Variables).
    - Secrets Plugin: Jenkins предоставляет плагин "Secrets" (Secrets Plugin),
    который позволяет хранить секреты в зашифрованном виде.
    - HashiCorp's Vault: Jenkins можно интегрировать с HashiCorp's Vault,
    который является системой управления секретами

    Чтобы использовать секреты в пайплайнах, можно использовать следующие методы:
    - Injecting Secrets: Секреты можно инъецировать в
    пайплайны с помощью плагина "Credentials Binding" (Credentials Binding Plugin).
    - Using Environment Variables: Секреты можно использовать
    в пайплайнах как переменные окружения.
    - Using the Secrets Plugin: Секреты можно использовать
    в пайплайнах с помощью плагина "Secrets" (Secrets Plugin)
    ...
    секреты хранятся в виде учетных данных (credentials) и инъецируются
    в пайплайн с помощью плагина "Credentials Binding" (Credentials Binding Plugin)
```

```txt
2 - Как запустить определенную стадию пайплайна в Docker-контейнере?

->  Можно использовать директиву docker в Jenkinsfile для запуска
    стадий пайплайна в Docker-контейнерах

    stages {
        stage('Build') {
            docker.image('maven:3.6.0').inside {
                sh 'mvn clean package'
            }
    
    Можно использовать плагин "Docker Pipeline" (Docker Pipeline Plugin)
    для запуска стадий пайплайна в Docker-контейнерах
    ...
    Стадия "Build" будет запущена в Docker-контейнере с
    образом maven:3.6.0. Аргументы -v /tmp:/tmp позволяют 
    монтировать директорию /tmp из контейнера в директорию /tmp на хосте

    - clean: удаляет все файлы и директории, которые были
    созданы во время предыдущей сборки.
    - package: собирает проект в виде JAR-файла или WAR-файла,
    в зависимости от типа проект
```
...
->  Определенные шаги пайплайна будут выполнены в любом случае, независимо
    от того, успешно завершился пайплайн или нет, можно использовать директиву
    post в Jenkinsfile

    Директива post позволяет указать шаги, которые должны быть выполнены
    после завершения пайплайна, независимо от того, успешно завершился
    пайплайн или нет.

    post {
        always {
            sh 'echo "Пайплайн завершился"'
        }
        success {
            sh 'echo "Пайплайн завершился успешно"'
        }
        failure {
            sh 'echo "Пайплайн завершился с ошибкой"'
        }
    }

    - always: шаги, которые должны быть выполнены в
    любом случае, независимо от того, успешно завершился пайплайн или нет.
    - success: шаги, которые должны быть выполнены только
    в том случае, если пайплайн завершился успешно.
    - failure: шаги, которые должны быть выполнены только
    в том случае, если пайплайн завершился с ошибкой.
```

```txt
4 - Что означает статус билда UNSTABLE?

->  UNSTABLE - это статус, который указывает на то, что билд не
    завершился успешно, но и не завершился с ошибкой. Это может
    быть вызвано различными причинами, такими как:
        - Предупреждения во время компиляции или сборки
        - Нестабильное поведение приложения
        - Неполные или неудачные тесты
        - Другие проблемы, которые не являются критическими, но
        могут повлиять на качество билда
    - Когда билд имеет статус UNSTABLE, это не обязательно означает,
    что билд не может быть использован, но это может указывать на то,
    что билд требует дополнительного внимания или исправления
    - Чтобы исправить статус UNSTABLE, необходимо проанализировать лог
    билда и исправить проблемы, которые вызвали этот статус
```

```txt
5 - Как запускать пайплайн по вебхуку?

->  1 - Создать вебхук: Создайте вебхук в Jenkins, который
    будет вызываться при получении HTTP-запроса.
    2 - Определить URL вебхука: Определите URL вебхука, который
    будет использоваться для вызова пайплайна.
    3 - Настроить пайплайн: Настройте пайплайн, чтобы он мог
    быть вызван по вебхуку.
    4 - Создать токен: Создайте токен, который будет использоваться
    для аутентификации вебхука.
    5 - Настроить вебхук: Настройте вебхук, чтобы он мог вызывать
    пайплайн по URL, который вы определили.

    URL вебхука:
    http://your-jenkins-server.com/jenkins/job/your-pipeline-name/buildWithParameters

    Токен: your-token

    Параметры: param1=value1&param2=value2

    Чтобы вызвать пайплайн по вебхуку, необходимо отправить
    HTTP-запрос на URL вебхука с токеном и параметрами
    ...
    После отправки HTTP-запроса, пайплайн будет вызван и начнет выполняться
    Плагин "Webhook" в Jenkins, чтобы упростить процесс
    создания и настройки вебхука
```
...
->  pipeline {
    ...
    можно использовать плагин "Parallel Pipeline" в Jenkins,
    чтобы упростить процесс создания и настройки параллельных
    стадий пайплайна
```
...
->  jenkins-cli build <job-name>
    <job-name> - это имя пайплайна, который вы хотите запустить

    jenkins-cli build <job-name> -p <parameter-name>=<parameter-value>
    <parameter-name> - это имя параметра, 
    а <parameter-value> - это значение параметра.

    jenkins-cli build my-pipeline -p BRANCH=main -p ENVIRONMENT=dev

    jenkins-cli build <job-name> -f <parameter-file>
    <parameter-file> - это имя файла, который содержит параметры пайплайна.
```

```txt
2 - Как запускать Jenkins-пайплайн из другого Jenkins-пайплайна?

->  Для этого необходимо установить плагин "Parameterized Trigger Plugin" в Jenkins.
    - Затем необходимо зайти в настройки пайплайна,
    который необходимо запустить, и нажать на кнопку "Параметры".
    - В разделе "Параметры" необходимо добавить параметр
    "Trigger another Jenkins project".
    - В поле "Project name" необходимо ввести имя пайплайна,
    который необходимо запустить.
    - В поле "Parameters" необходимо указать параметры,
    которые необходимо передать пайплайну.
    - Нажать на кнопку "Сохранить".
    - После этого, когда пайплайн будет запущен, он автоматически
    запустит другой пайплайн с заданными параметрами.
-----------------------------------------------------------------------------
    Использование API Jenkins
    Чтобы запустить Jenkins-пайплайн из другого Jenkins-пайплайна с
    помощью API Jenkins, необходимо выполнить следующие шаги:
        - Получение токена API Jenkins:
        - Зайти в Jenkins и зайти в настройки.
        - Перейти в раздел "Системные конфигурации".
        - Перейти в раздел "API".
        - Нажать на кнопку "Получить токен".
        - Скопировать полученный токен.
    Создать команду curl, которая будет отправлять POST-запрос на API Jenkins.
    В команде необходимо указать:
        -X POST - для отправки POST-запроса.
        -H "Authorization: Bearer <токен>" - для указания токена API Jenkins.
        -H "Content-Type: application/json" - для указания типа содержимого запроса.
        -d '{"parameter": {"name": "PARAM_NAME", "value": "PARAM_VALUE"}}' - для
        указания параметров, которые необходимо передать пайплайну.
        http://<адрес_дженкинс>/job/<имя_пайплайна>/build - для указания адреса
        Jenkins и имени пайплайна, который необходимо запустить

    curl -X POST -H "Authorization: Bearer <токен>" -H "Content-Type:
    application/json" -d '{"parameter": {"name": "PARAM_NAME", "value":
    "PARAM_VALUE"}}' http://<адрес_дженкинс>/job/<имя_пайплайна>/build
-----------------------------------------------------------------------------
    Использование шага build: Вы можете использовать шаг build
    в Jenkinsfile для запуска другого пайплайна.
    pipeline {
    agent any
    stages {
        stage('Запуск другого пайплайна') {
            steps {
                build job: 'имя-другого-пайплайна', parameters: 
                [string(name: 'параметр', value: 'значение')]
            }
        }
    }
    }
-----------------------------------------------------------------------------
    Использование Jenkins CLI: Вы можете использовать Jenkins CLI для
    ...
    pipeline {
        ...
        stages {
            stage('Запуск другого пайплайна') {
                steps {
                    sh 'jenkins-cli build имя-другого-пайплайна'
                }
            }
        }
    }   
